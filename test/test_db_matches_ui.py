...
def test_contact_list(app, orm):
    ...
    orm_list = sorted(map(clean, orm.get_contact_list_full()), key=Contact.id_or_max)

    for ui, orm in zip(ui_list, orm_list):
        assert ui.id == orm.id
        assert ui.firstname == orm.firstname
        assert ui.lastname == orm.lastname
        assert ui.all_phones == orm.all_phones
        assert ui.all_emails == orm.all_emails
